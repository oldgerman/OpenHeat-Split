...
TIPCONFIG MyTip[MaxTipConfig] = {
    {
        ...
        {
            {2.5, 0, 0.15}, //上电瞬间防止开关电源短路保护的预备参数
            {30.0, 0, 0.5}, 
            {20.0, 0, 0.5}
        }, 
        "智能加热台"
...
void LoadTipConfig(void)
{
    ...
    //载入PID参数
    readyKp = MyTip[TipID].PID[0][0];
    readyKi = MyTip[TipID].PID[0][1];
    readyKd = MyTip[TipID].PID[0][2];

    aggKp = MyTip[TipID].PID[1][0];
    aggKi = MyTip[TipID].PID[1][1];
    aggKd = MyTip[TipID].PID[1][2];

    consKp = MyTip[TipID].PID[2][0];
    consKi = MyTip[TipID].PID[2][1];
    consKd = MyTip[TipID].PID[2][2];
    //载入名称
...
void SaveTipConfig(void)
{
    ...
    //载入PID参数
    MyTip[TipID].PID[0][0] = readyKp;
    MyTip[TipID].PID[0][1] = readyKi;
    MyTip[TipID].PID[0][2] = readyKd;

    MyTip[TipID].PID[1][0] = aggKp;
    MyTip[TipID].PID[1][1] = aggKi;
    MyTip[TipID].PID[1][2] = aggKd;

    MyTip[TipID].PID[2][0] = consKp;
    MyTip[TipID].PID[2][1] = consKi;
    MyTip[TipID].PID[2][2] = consKd;
