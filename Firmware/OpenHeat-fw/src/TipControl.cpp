...
double *ptr_PID_Setpoint = &PID_Setpoint;
double TempGap = 0;
uint32_t ADCSamplingInterval = 0; //ADC采样间隔(ms)
//PID
float readyKp = 1.0, readyKi = 0, readyKd = 0.05;    //用于避免刚上电热床还没温升阻值过小导致开关电源短路保护的预备PID参数
float aggKp = 7.0, aggKi = 0, aggKd = 0.2;
float consKp = 30.0, consKi = 1, consKd = 0.5;
...
void TemperatureControlLoop(MAX6675 *thermocouple)
{
     ...
     if (SYS_Ready)
    {
        ...
        ADCSamplingInterval = millis() - ADCSamplingTime;
        if (ADCSamplingInterval < ADC_PID_Cycle * (9 / 10.0)) goto gg; //9/10周期ADC不应该工作，应该把时间留给加热
        ...
        if(millis() - ADCReadCoolTimer <= ADC_PID_Cycle / 10)
        {
         ...
         //读取并平滑滤波经过运算放大器放大后的热偶ADC数据
    TipTemperature = thermocouple->readCelsius();
    ...
    gg: printf("Temp:%lf, ptr_PID_Setpoint:%lf\r\n", TipTemperature, *ptr_PID_Setpoint);
    // if (ADC != -1)
    // TipTemperature = thermocouple->readCelsius();
    if(TipTemperature != 0)
    {
        ...
        TempGap = abs(PID_Setpoint - TipTemperature);   //求绝对值
        ...
        if (PIDMode)
        {
            ...
            if(TipTemperature < ReadyTemp) {
                offset_ReadyTemp = ReadyTemp + 20;
                ptr_PID_Setpoint = &offset_ReadyTemp;
                MyPID.SetTunings(readyKp, readyKi, readyKd);
            }else {
                ptr_PID_Setpoint = &PID_Setpoint;
                if (TempGap < 30) {
